...
/* ─────────────────────────────────────────────────────
   ...
   Public — lets a customer look up their own booking
   history using just the phone number they originally
   booked with. No OTP/password — see README for the
   security tradeoff this accepts and why.

   Phone numbers are matched on digits only, so "98765
   43210", "9876543210", and "+91 98765-43210" all match
   the same underlying booking, regardless of how the
   customer happened to type it either time.

   Deliberately returns a minimal field set — never
   adminNote, never internal status-change metadata —
...
exports.getBookingsByPhone = async (req, res) => {
  try {
    ...
    // Match the last 10 digits regardless of how they're separated in
    // storage. The phone field allows spaces/dashes per the Booking schema
    // (e.g. "98765 43210"), so a naive "last 10 contiguous characters"
    // regex would miss real bookings whenever the original submission had
    // any internal formatting. Instead, build a pattern that allows zero
    // or more non-digit characters between each digit — this means
    // "9876543210", "98765 43210", and "98765-43210" are all correctly
    // treated as the same number, however either side happened to type it.
    const lastTenDigits = digitsOnly.slice(-10);
    const flexiblePattern = lastTenDigits.split('').join('\\D*') + '$';

    const bookings = await Booking.find({
      phone: { $regex: flexiblePattern },
    })
      .sort({ createdAt: -1 })
      .select('name service preferredDate preferredTime stylistName status createdAt')
      .lean();

    return res.json({ success: true, data: bookings });
  } catch (err) {
    console.error('[getBookingsByPhone]', err);
    return res.status(500).json({ success: false, message: 'Could not look up bookings.' });
  }
};
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    // Awaited, but never allowed to throw or slow this response down by
    // much — WhatsApp's API typically responds in well under a second, and
    // wrapping in try/catch means a Meta outage or misconfigured template
    // can never turn into a failed status update. The outcome (sent /
    // simulated / failed-with-reason) is returned to the caller so the
    // admin dashboard can show real status instead of just hoping it
    // worked — exactly the kind of invisible-feedback gap that caused so
    // much back-and-forth earlier in getting Mongo/Render set up correctly.
    let whatsappResult = null;
    if (status === 'confirmed') {
      try {
        whatsappResult = await whatsapp.sendBookingConfirmed(booking);
      } catch (err) {
        console.error('[updateBooking] WhatsApp confirmation send failed:', err.message);
        whatsappResult = { sent: false, simulated: false, error: err.message };
      }
    } else if (status === 'completed') {
      try {
        whatsappResult = await whatsapp.sendReviewRequest(booking, process.env.GOOGLE_REVIEW_URL);
      } catch (err) {
        console.error('[updateBooking] WhatsApp review request failed:', err.message);
        whatsappResult = { sent: false, simulated: false, error: err.message };
      }
    }

    return res.json({ success: true, data: booking, whatsapp: whatsappResult });
  } catch (err) {
